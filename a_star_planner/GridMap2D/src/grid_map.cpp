...
GridMap2D::GridMap2D(double resolution, const Eigen::Vector2i& world_size)
    : resolution_(resolution), inflate_radius_(0.5) {  // 默认膨胀半径0.5米
    // 1. 校验输入合法性
    if (resolution <= 0) {
        throw std::invalid_argument("地图分辨率必须为正数（当前：" + std::to_string(resolution) + "）");
    }
    if (world_size.x() <= 0 || world_size.y() <= 0) {
        throw std::invalid_argument("世界尺寸（x,y）必须为正数（当前：(" + 
            std::to_string(world_size.x()) + "," + std::to_string(world_size.y()) + ")）");
    }

    // 2. 计算栅格数量（世界尺寸 / 分辨率，向上取整确保覆盖完整世界范围）
    int grid_cols = static_cast<int>(std::ceil(static_cast<double>(world_size.x()) / resolution));  // x方向→列数
    int grid_rows = static_cast<int>(std::ceil(static_cast<double>(world_size.y()) / resolution));  // y方向→行数
    map_size_.x() = grid_cols;
    map_size_.y() = grid_rows;

    // 3. 设置地图原点（世界坐标）：让地图中心对齐世界坐标原点
    origin_.x() = -static_cast<double>(world_size.x()) / 2.0;  // 世界x范围：[origin.x(), origin.x()+world_size.x()]
    origin_.y() = -static_cast<double>(world_size.y()) / 2.0;  // 世界y范围：[origin.y(), origin.y()+world_size.y()]
    ...
    std::cout << "  - 世界尺寸：" << world_size.x() << "m × " << world_size.y() << "m" << std::endl;
    std::cout << "  - 栅格分辨率：" << resolution << "m/栅格" << std::endl;
    std::cout << "  - 栅格数量：" << grid_cols << "列 × " << grid_rows << "行" << std::endl;
    std::cout << "  - 世界坐标范围：x∈[" << std::fixed << std::setprecision(2) << origin_.x() << "," 
              << origin_.x() + world_size.x() << "], y∈[" << origin_.y() << "," 
              << origin_.y() + world_size.y() << "]" << std::endl;
...
// Getter：获取默认膨胀半径
// double GridMap2D::inflateRadius() const {
